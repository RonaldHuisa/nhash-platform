...
async function ensureUserHashPoints(clientOrPool, userId) {
  ...
  const result = await clientOrPool.query(
    ...
    ON CONFLICT (user_id) DO UPDATE SET
      max_bonus_percent = GREATEST(user_hash_points.max_bonus_percent, EXCLUDED.max_bonus_percent),
...
async function syncValidReferralPoints(clientOrPool, userId) {
  ...
  const validResult = await clientOrPool.query(
    `
    WITH direct_referrals AS (
      SELECT
        ...
        GREATEST(
          ...
          COALESCE(r.balance_usdt, 0),
          COALESCE(r.recharge_balance_usdt, 0)
        ) AS invested_amount
      FROM users r
      LEFT JOIN mining_accounts ma ON ma.user_id = r.id
      WHERE r.referred_by_id = $1
    ), new_valid AS (
      SELECT dr.*
      FROM direct_referrals dr
      WHERE dr.invested_amount >= $2
        AND NOT EXISTS (
          SELECT 1
          FROM hash_point_referrals hpr
    ...
    SELECT COUNT(*)::int AS added_points FROM inserted
    `,
    [userId, MIN_VALID_INVESTMENT_USDT]
  );

  const addedPoints = Number(validResult.rows[0]?.added_points || 0);
...
async function getHashRewardsStatusForUser(clientOrPool, userId, options = {}) {
  ...
  const statsResult = await clientOrPool.query(
    `
    SELECT
      COUNT(*)::int AS valid_referrals,
      COALESCE(SUM(invested_amount), 0) AS valid_referrals_invested
  ...
  const directResult = await clientOrPool.query(
    ...
    SELECT
      ...
      )::int AS currently_valid_direct_referrals
    FROM users r
    LEFT JOIN mining_accounts ma ON ma.user_id = r.id
    WHERE r.referred_by_id = $1
    `,
  ...
  return {
    addedPoints,
    validReferrals: Number(statsResult.rows[0]?.valid_referrals || 0),
    ...
    currentlyValidDirectReferrals: Number(directResult.rows[0]?.currently_valid_direct_referrals || 0),
